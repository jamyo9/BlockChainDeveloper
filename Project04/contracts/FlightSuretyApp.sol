...
contract FlightSuretyApp {
    ...
    // Custom constants
    uint8 private constant AMOUNT_AIRLINE_MINIMUM = 10;
    uint8 private constant MINIMUM_AIRLINES_CONSENSUS = 4;
    ...
    *
    */
    function registerAirline(address airline, string name) external returns(bool success, uint256 votes) {
        require(airline != address(0), "'airline' must be a valid address.");

        return _flightSuretyData.registerAirline(msg.sender, airline, name);
    }
    ...
    */
    function registerFlight(string memory flight, uint256 timestamp) public returns (bytes32) {
        return _flightSuretyData.registerFlight(msg.sender, flight, timestamp);
    }
    ...
    */
    function processFlightStatus(address airline, string memory flight, uint256 timestamp, uint8 statusCode) internal {
        _flightSuretyData.processFlightStatus(airline, flight, timestamp, statusCode);
    ...
    function getFlightKey(address airline,
                        string flight,
                        uint256 timestamp) internal returns(bytes32) {
