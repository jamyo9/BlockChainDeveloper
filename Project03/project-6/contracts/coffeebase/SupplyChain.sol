...
contract SupplyChain is Ownable, ConsumerRole, DistributorRole, FarmerRole, RetailerRole {
  ...
  // Define 'supplyChainOwner'
  address payable supplyChainOwner;
  ...
  modifier onlyOwner() {
    require(msg.sender == supplyChainOwner, "The Sender is not the Owner");
    _;
  }
  ...
  modifier paidEnough(uint _price) {
    require(msg.value >= _price, "The amount does not cover the price");
    _;
  }
  ...
  modifier harvested(uint _upc) {
    require(items[_upc].itemState == State.Harvested, "The item status is not the Harvested");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Processed
  modifier processed(uint _upc) {
    require(items[_upc].itemState == State.Processed, "The item status is not the Processed");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Packed
  modifier packed(uint _upc) {
    require(items[_upc].itemState == State.Packed, "The item status is not the Packed");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is ForSale
  modifier forSale(uint _upc) {
    require(items[_upc].itemState == State.ForSale, "The item status is not the ForSale");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Sold
  modifier sold(uint _upc) {
    require(items[_upc].itemState == State.Sold, "The item status is not the Sold");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Shipped
  modifier shipped(uint _upc) {
    require(items[_upc].itemState == State.Shipped, "The item status is not the Shipped");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Received
  modifier received(uint _upc) {
    require(items[_upc].itemState == State.Received, "The item status is not the Received");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Purchased
  modifier purchased(uint _upc) {
    require(items[_upc].itemState == State.Purchased, "The item status is not the Purchased");
    _;
  }

  // In the constructor set 'supplyChainOwner' to the address that instantiated the contract
  // and set 'sku' to 1
  // and set 'upc' to 1
  ...
  function harvestItem(
        uint _upc, address payable _originFarmerID, string memory _originFarmName,
  ...
  function buyItem(uint _upc) public payable forSale(_upc) paidEnough(items[_upc].productPrice)
    ...
    address payable _ditributorID = msg.sender;
    DistributorRole.addDistributor(_ditributorID);

    items[_upc].ownerID.transfer(items[_upc].productPrice);
    items[_upc].ownerID = _ditributorID;
    items[_upc].distributorID = _ditributorID;
  ...
  function receiveItem(uint _upc) public shipped(_upc) onlyRetailer()
    ...
    address payable _retailerID = msg.sender;
  ...
  function purchaseItem(uint _upc) public received(_upc)
    ...
    address payable _consumerID = msg.sender;
